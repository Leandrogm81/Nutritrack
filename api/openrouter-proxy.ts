  ...
  try {
    ...
    try {
      ...
      if (error instanceof Error && error.name === 'AbortError') {
        console.error(`OpenRouter proxy timeout after ${timeoutMs}ms`);
        return jsonResponse({
      ...
      console.error('OpenRouter upstream fetch failed:', error);
      return jsonResponse({
    ...
    console.log(`OpenRouter proxy responded with ${proxyRes.status} in ${Date.now() - startedAt}ms`);
